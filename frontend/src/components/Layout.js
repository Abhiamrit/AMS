...
function NavItem({ to, icon: Icon, label, onClick }) {
  if (onClick)
    return (
      <button
        onClick={onClick}
        className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-navy-200 hover:bg-navy-600 hover:text-white w-full text-left text-sm transition-colors"
      >
        <Icon size={16} />
        <span>{label}</span>
      </button>
  ...
  return (
    <NavLink
      to={to}
        ...
        to === "/admin" || to === "/staff" || to === "/mas" || to === "/section"
      }
      className={({ isActive }) =>
...
const NAV_LINKS = {
  ...
  // Section head sees section inbox in addition to staff links
  STAFF_HEAD: [
    { to: "/staff", icon: LayoutDashboard, label: "Dashboard" },
    { to: "/staff/clients", icon: Building2, label: "My Clients" },
    { to: "/staff/assignments", icon: ClipboardList, label: "My Assignments" },
    { to: "/section/inbox", icon: Inbox, label: "Section Inbox" },
    { to: "/section", icon: ScrollText, label: "Section History" },
  ],
  // Section member (STAFF with section but not section head) who is assigned work
  STAFF_MEMBER: [
    { to: "/staff", icon: LayoutDashboard, label: "Dashboard" },
    { to: "/staff/clients", icon: Building2, label: "My Clients" },
    { to: "/staff/assignments", icon: ClipboardList, label: "My Assignments" },
    { to: "/section/work", icon: Briefcase, label: "My Work" },
  ],
  MAS: [{ to: "/mas", icon: LayoutDashboard, label: "MAS Dashboard" }],
};
...
export default function Layout() {
  ...
  const handleNotifClick = async (notif) => {
    ...
    if (notif.related_assignment_id) {
      ...
      if (role === "MAS") {
        navigate(`/mas/assignments/${notif.related_assignment_id}`);
      } else if (role === "STAFF") {
        // Section head — go to section inbox so they can assign
        if (user.section_id && user.is_section_head) {
          navigate("/section/inbox");
          // Section member — go to My Work list
        } else if (user.section_id && !user.is_section_head) {
          navigate("/section/work");
          // Plain staff creator — go to assignment detail
        } else {
          navigate(`/staff/assignments/${notif.related_assignment_id}/detail`);
        }
      } else {
  ...
  let navKey = user?.role;
  if (user?.role === "STAFF") {
    if (user.section_id && user.is_section_head) navKey = "STAFF_HEAD";
    else if (user.section_id && !user.is_section_head) navKey = "STAFF_MEMBER";
    else navKey = "STAFF";
  }
  const links = NAV_LINKS[navKey] || NAV_LINKS[user?.role] || [];

  return (
    <div className="flex h-screen bg-navy-50 overflow-hidden">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-0 overflow-hidden"} bg-navy-700 flex-shrink-0 flex flex-col transition-all duration-200`}
      >
        {/* Logo */}
        ...
        <div className="px-4 py-3 border-b border-navy-600">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <span className={`badge-${user?.role?.toLowerCase()} mt-1`}>
              {user?.role}
            </span>
            {user?.is_section_head === 1 && (
              <span className="flex items-center gap-0.5 text-xs text-amber-300 mt-1">
                <Shield size={11} /> Head
              </span>
            )}
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-navy-100 h-14 flex items-center justify-between px-4 flex-shrink-0 shadow-sm">
          <button
            ...
            className="text-navy-600 hover:text-navy-800 p-1 rounded"
          >
