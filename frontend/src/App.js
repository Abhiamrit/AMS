...
import RouteSetup from "./pages/staff/RouteSetup";

import MasDashboard from "./pages/mas/MasDashboard";
import MasReview from "./pages/mas/MasReview";

import SectionInbox from "./pages/section/SectionInbox";
import AssignmentWork from "./pages/section/AssignmentWork";
import MyWork from "./pages/section/MyWork";
import SectionHeadDashboard from "./pages/section/SectionHeadDashboard";
...
function RequireAuth({ children, roles }) {
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center h-screen text-navy-700">
        Loading…
      </div>
    );
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          >
            ...
            <Route path="assignments/:id/route" element={<RouteSetup />} />
            <Route
              path="assignments/:id/detail"
              element={<AssignmentDetail />}
            />
            {/* Route setup page — reuses CreateAssignment step 2 via navigate state,
                but AssignmentDetail handles the full detail. Section routing
                is embedded in CreateAssignment's step 2 flow. */}
          </Route>

          {/* SECTION pages — accessible by STAFF users with a section */}
          <Route
            path="/section"
            element={
              <RequireAuth roles={["STAFF"]}>
          ...
          >
            ...
            <Route path="work/:routingId" element={<AssignmentWork />} />
          </Route>
