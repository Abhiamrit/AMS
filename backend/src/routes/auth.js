  ...
  req.session.regenerate((err) => {
    ...
    res.json({
      user: {
        ...
        is_section_head: user.is_section_head || 0,
      }
    });
  });
});
...
router.get('/me', requireAuth, (req, res) => {
  const { user_id, username, full_name, role, section_id, is_section_head } = req.user;
  res.json({ user: { user_id, username, full_name, role, section_id: section_id || null, is_section_head: is_section_head || 0 } });
});
