...
export default function MasReview() {
  ...
  const exportPDF = () => {
    window.open(`${process.env.REACT_APP_API_URL}/export/${id}/pdf`, "_blank");
  };
  const exportExcel = () => {
    window.open(
      `${process.env.REACT_APP_API_URL}/export/${id}/excel`,
      "_blank",
    );
  };
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            {[
              ...
              [
                "Client",
                `${assignment.client_name} (${assignment.client_type})`,
              ],
        ...
        {assignment.columns?.length > 0 && (
          <div className="card p-6">
              ...
              <table className="min-w-full text-xs">
                <thead>
                  <tr className="bg-navy-700">
                    <th className="px-3 py-2.5 text-left text-white font-medium">
                      #
                    </th>
                    {assignment.columns.map((col) => (
                      <th
                        key={col.column_id}
                        className="px-3 py-2.5 text-left text-white font-medium"
                      >
                        <div>{col.column_name}</div>
                        <div className="text-navy-300 text-xs font-normal">
                          {col.column_type}
                        </div>
                      </th>
                    ))}
                ...
                <tbody>
                  {assignment.rows.length === 0 && (
                    <tr>
                      <td
                        colSpan={assignment.columns.length + 1}
                        ...
                        return (
                          ...
                          >
                            {col.column_type === "CHECKBOX" ? (
                              <span
                                className={`inline-block w-4 h-4 rounded border ${val === "true" ? "bg-emerald-500 border-emerald-600" : "border-gray-300"}`}
                              />
                            ) : (
            ...
            <p className="text-xs text-gray-400 mt-2">
              {assignment.columns.length} columns · {assignment.rows.length}{" "}
              rows
            </p>
          </div>
        )}
        ...
        <div className="card p-6">
          ...
          {isCompleted ? (
            <div className="bg-emerald-50 border border-emerald-200 rounded p-4 text-sm text-gray-700 whitespace-pre-wrap">
              {assignment.mas_remarks || (
                <span className="text-gray-400 italic">No remarks added.</span>
              )}
            </div>
          ) : (
            <>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-amber-700 mt-0.5">
                  This will notify the originating staff user and close the
                  assignment.
                </div>
              </div>
              <button
        ...
        {isCompleted && (
          <div className="card p-4 bg-emerald-50 border-emerald-200 flex items-center gap-3">
            ...
            <div className="text-sm text-emerald-800">
              ...
              <strong>{formatDateTime(assignment.completed_at)}</strong>
              {assignment.completed_by_name &&
                ` by ${assignment.completed_by_name}`}
              .
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              ...
              <strong>{assignment.assignment_code}</strong> as Completed? This
