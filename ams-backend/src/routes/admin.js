...
router.get('/stats', (req, res) => {
  ...
  `).all();
  res.json({ totalAssignments, pending, completed, draft, totalClients, totalStaff, totalMas, recentActivity });
});
...
router.post('/users', async (req, res) => {
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `).run(username.trim(), hash, full_name.trim(), role, req.user.user_id);
